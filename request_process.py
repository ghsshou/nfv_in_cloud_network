        ...
        if vnf_type in unmaped_vnf and index > 0:
            ...
            # second, if the previous one cannot be used, then check overall network
                ...
                if other_vm:
                    ...
                    new_vm = create_vm_for_vnf(data_base, index, vnf_type, req_vnfs, req, est_time, other_vnf_pro_times, cand_dc)
...
def check_vms_for_vnf(vms, vnf_type, est_time, req):
    ...
    if vnf_type.value[1] == 0:  # single thread
        ...
        for vm in vms:
            ...
            if est_time[vnf_type] + process_time + install_time < req.deadline:
                cand_vms.append(vm)
    final_vm = None
    cpu_cores = 0
    for vm in cand_vms:
